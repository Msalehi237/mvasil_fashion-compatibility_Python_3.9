...
def main():
    ...
    checkpoint = torch.load('runs/%s/' % (args.name) + 'model_best.pth.tar', encoding='latin1')
    tnet.load_state_dict(checkpoint['state_dict'])
    test_acc = test(test_loader, tnet)
...
def train(train_loader, tnet, criterion, optimizer, epoch):
    ...
    tnet.train()
    for batch_idx, (img1, desc1, has_text1, img2, desc2, has_text2, img3, desc3, has_text3, condition) in enumerate(
            train_loader):
